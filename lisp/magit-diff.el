...
(cl-defmethod transient-set-value ((obj magit-diff-refresh-prefix))
  (magit-diff--set-value obj))

(cl-defmethod transient-save-value ((obj magit-diff-refresh-prefix))
...
(defun magit-diff--get-value (mode &optional use-buffer-args)
  ...
  (let (args files)
    ...
    (when (get mode 'magit-limit-to-visited-file)
...
(defun magit-diff--set-value (obj &optional save)
  (let* ((obj (oref obj prototype))
         (key (intern (format "magit-diff:%s" major-mode)))
         (val (transient-args)))
    (put major-mode 'magit-log-current-arguments val)
    (when save
      (setf (alist-get key transient-values) val)
      (transient-save-values))
    (transient--history-push obj)
    (pcase-let ((`(,args ,alist) (-separate #'atom val)))
...
(put 'magit-diff-mode 'magit-limit-to-visited-file t)
...
(put 'magit-revision-mode 'magit-limit-to-visited-file t)
...
(put 'magit-merge-preview-mode 'magit-limit-to-visited-file nil)
