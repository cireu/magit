...
(cl-defmethod transient-set-value ((obj magit-log-refresh-prefix))
  (magit-log--set-value obj))

(cl-defmethod transient-save-value ((obj magit-log-refresh-prefix))
...
(defun magit-log--set-value (obj &optional save)
  (let* ((obj (oref obj prototype))
         (key (intern (format "magit-log:%s" major-mode)))
         (val (transient-args)))
    ...
    (pcase-let ((`(,args ,alist) (-separate #'atom val)))
      ...
      (unless (derived-mode-p 'magit-log-select-mode)
        (setq magit-buffer-log-files (cdr (assoc "--" alist)))))
    (magit-refresh)))
